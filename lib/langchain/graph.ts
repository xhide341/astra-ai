...
const TOTAL_GRAPH_ITERATIONS = 3.5;
...
async function teacherNode(state: typeof StateAnnotation.State) {
  ...
  // Provide context for teacher's response
  const isFirstTurn = state.iteration === 0;
  const inputMessage = isFirstTurn
    ? `A student is asking: "${lastMessage}"\n\nProvide an engaging explanation with interesting details or context.`
    : `Your student responds: "${lastMessage}"\n\nAnswer their question or build on their observation with helpful details.`;
...
async function facilitatorNode(state: typeof StateAnnotation.State) {
  ...
  // No need for final iteration check since conversation ends after teacher's response
  const inputMessage = `The professor just said: "${teacherMessage}"\n\nRespond with genuine curiosity - ask a follow-up question, request more details, or share an interesting observation about what they explained.`;
...
// Define when to continue or end conversation after teacher responds
function shouldContinueAfterTeacher(
  state: typeof StateAnnotation.State,
): "__end__" | "facilitator" {
  if (state.iteration >= TOTAL_GRAPH_ITERATIONS) {
    console.log("🏁 Conversation Complete - Ending after teacher's response");
    state.iteration = 0;
    return "__end__";
  }
  console.log("➡️ Continuing to Facilitator");
  return "facilitator";
}

// Create the graph workflow - check end condition after teacher speaks
const workflow = new StateGraph(StateAnnotation)
  .addNode("teacher", teacherNode)
  .addNode("facilitator", facilitatorNode)
  .addEdge("__start__", "teacher")
  .addConditionalEdges("teacher", shouldContinueAfterTeacher)
  .addEdge("facilitator", "teacher");
