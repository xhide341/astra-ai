...
const Conversation = () => {
  ...
  const fullFilteredMessages = messages.filter(
    (message) => message.chatId === activeChat?.id,
  );

  // Hide the first user message (which is just the topic configuration)
  const filteredMessages = fullFilteredMessages.filter((message, index) => {
      ...
      {isStreaming && currentStreamedContent && activeChat && (
        <div className="flex items-start gap-x-2">
          ...
          >
            ...
            >
              {processContent(currentStreamedContent)}
            </ReactMarkdown>
          </div>
        </div>
      )}
